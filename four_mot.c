...
/*****************************************************************/
{
	...
	/* 17.01.1994: include logging on file porta.log */
	log( "| iter- | %10s |  # ineq  |   max| long|   non- |     mem |  cpu-time | real-time |%s\n",hest1,oestr1);
	log( "| ation | %10s |          |  bit-|arith|  zeros |    used |      used |      used |%s\n",hest2,oestr2);
	log( "|       | %10s |          |length|metic|   in %c |   in kB |    in sec |    in sec |%s\n",hest3,'%',oestr3);
	log( "|-------|------------|----------|------|-----|--------|---------|-----------|-----------|%s\n",oestr4);
}


/*****************************************************************/
void gauss(traf,sysrow,eqrl,dim,equa_in,ineq,
	   equa,indx)
int traf,sysrow,eqrl,dim, equa_in;
int *ineq, *equa;
int indx[];
/*****************************************************************/
/*
 ...
 * Each row i is also accessible via list[i]->sys.
 ...
 *    sysrow-equa_in.
 * The rows of "ar2" are also accessible via list[i]->sys
 * for i = 0,...,ineq_in-equa-1.
 * "ineq" is not changed, if traf=1.
 */
{ 
	float t,time_used(), total_time();
	struct list *iep;
	register int n,nn;
	int col,row,pivcol;
	int i,j, ineq_in, nieq;
	RAT *sysp,*sptr,*pivot;
	unsigned *newmark,nmark;
	int ld,nz;
	char *allo();
	void debug(), allo_list();
	...
	for (itr = equa_in; itr != 0; itr--) 
	{
		...
		{ 
			...
			/* 17.01.1994: include logging on file porta.log */
			log( " variable %d eliminated - finite equation found\n",
				 indx[pivcol]+1 );
		}
		/* 
		 * For row = 1,..,ineq_in-equa-1,
		 * eliminate variable pivcol in list[row]->sys by adding list[0]->sys,
		 * store the result in list[ineq+row-1]->sys.
		 * The pivot variable is removed and list[ineq+row-1]->sys
		 * shortened by 1.
		 */
		for (row = 1; row < ineq_in-*equa; row++) 
		{
			gauss_calcnewrow(list[0]->sys,list[row]->sys,pivcol,
							 list[(*ineq)+row-1]->sys,1,sysrow);
		}

		if (indx[pivcol] < 0 && traf) 
		{ 
			/* 
			 * Create a new inequality in list[*ineq+ineq_in-equa-1]->sys,
			...
			for (col = 0,i = 0; col < sysrow; col++)  
				if (col != pivcol) 
				{
					(*RAT_assign)(list[*ineq + ineq_in-*equa-1]->sys+i,
								  list[0]->sys+col);
					i++;
				}
			if (pivot->num < 0)
				for (col = 0,i = 0; col < sysrow; col++) 
					(list[*ineq + ineq_in-*equa-1]->sys+col)->num *= -1;
		}
		...
		/* 
		 * Copy list[*ineq]->sys, ..., list[*ineq + ineq_in-equa-1]->sys back
		 * to list[0]->sys, ..., list[ineq_in-equa-1]->sys.
		 */
		sptr = ar2;
		for (row = 1; row <= ineq_in-*equa; row++) 
		{
			(*RAT_row_prim)(list[*ineq+row-1]->sys,sptr,
							list[*ineq+row-1]->sys+sysrow-1,sysrow);
			list[row-1]->sys = sptr;
			sptr += sysrow;
		}
		...
		/* 17.01.1994: include logging on file porta.log */
		log( "|%6i |%11i |%9i |%5i |%4c |%7.2f |%8i |%10.2f |%10.2f |\n",
			itr, itr, ineq_in-itr+1, ld,MP_realised?'y':'n',
			(float)nz/(float)j, total_size/1000, time_used(),
			total_time());
	}
...
void fourier_motzkin(fname,nieq,rowl,niterat,poi_file,indx,elim_ord)
	 char fname[];
	 int nieq,rowl,niterat,poi_file,indx[],*elim_ord;
 ...
 * to be eliminated.
 * The output inequality i (0 <= i <= ineq-1) is in vector list[i]->sys.
 ...
 */
{ 
	float time_used(), total_time();
	struct list *iep;
	int elcol,col,maxnumineq = 0; 
	int sysrow,i,j,pos,zer,neg,new,p,n,nn,s=1,minineq, nel_xxx;
	 ...
	 * for the nieq vectors list[]->sys of length nieq+1,
	 */
	/*
	...
	if (rowl-niterat > FIRST_SYS_EL)
		msg("rowl too big or FIRST_SYS_EL too small");
	 ...
	 * list[i]->sys[1] is the beginning of the vector of those coefficients.
	 * list[i]->sys[0] will be used for other purposes.
	 * Actually, these vectors are stored in array "ar3",
	 * list[i]->sys only points to the respective position in this array.
	 * newmark is the same as list[i]->mark
	...
	for (itr = niterat; itr > 0; itr--) 
	{
		...
		if (!elim_ord && is_set(Opt_elim) && !MP_realised) 
		{
			...
			for (j = itr; j; j--) 
			{
				...
				for (i = 0; i < ineq; i++) 
				{
					vecpr(iesp, list[i]->sys+1, list[i]->sys, nieq);
					if (list[i]->sys[0].num < 0)
						(long) (list[i]->ptr)++;
				}
				
				for (cmbp = cmb, p = 0; p < ineq-1; p++, cmbp++) 
				{
					neg = (list[p]->sys[0].num < 0);
					pos = (list[p]->sys[0].num > 0);
					for (; *cmbp  && new < minineq ; cmbp++)
						if ((pos  && list[*cmbp]->sys[0].num < 0) ||
							(neg && list[*cmbp]->sys[0].num > 0)  )  new++;
				}
				if (new < minineq) 
				{
					minineq = new;
			...
			for (nn = i = 0; i < ineq; i++) 
			{
				if (!(long) (list[i]->ptr)) 
					nn++;
				else if ((long) (list[i]->ptr) < neg)
					neg = (long) (list[i]->ptr)/(sizeof(RAT));
			}
			if ((nf_dstf = nn) > finie) 
			{
				if (poi_file) 
					...
					for (i = ineq-1; nn > finie; i--) 
						if (!(long)(list[i]->ptr)) 
						{
							for (j = 0; j < rowl-niterat; j++)
								vecpr(ar2+nieq*(j+niterat), list[i]->sys+1, ar3+j, nieq);
		...
		/* 
		 * Compute the product of vectors "iesp" and list[i]->sys[1] 
		 * in list[i]->sys[0].
		 ...
		 * It is important for later that list[i]->sys < list[j]->sys
		...
		for(i = 0 ; i < ineq  ; i++)
			if ((list[i]->sys)->num > 0) 
			{
				/* rotate contents of list[pos,...,i] to the right */
				iep = list[i];
				for (j = i; j > pos; j--) 
					list[j] = list[j-1] ;
				list[pos++] = iep;
		...
		else
		{
			...
			/* 17.01.1994: include logging on file porta.log */
			log( "|%6i |%11i ",itr,(elim_ord?(neg-zer):neg)+zer*(ineq-neg));
		}
		 ...
		 */
		for (p = 0; p != zer; p++)
			(*RAT_row_prim)(list[p]->sys,list[p]->sys,list[p]->sys,sysrow);
		for (p = neg; p != ineq; p++)
			(*RAT_row_prim)(list[p]->sys,list[p]->sys,list[p]->sys,sysrow);
		 ...
		 * then list[new]->sys is list[p]->sys
		 * and  list[new]->ptr is list[n]->sys
		 */
		if (option & Chernikov_rule_off) 
			...
			for (p = 0; p != zer; p++)     
				...
				{
					allo_list(new,&newmark,blocks);
					list[new]->sys = list[p]->sys;
					list[new]->ptr = list[n]->sys;
					new++;    
				}
		else 
		{
					...
					*newmark = 0;
					...
					for (nn = ineq; nn != new; nn++) 
					{
						...
						if (*o1mp++ >= *nmp++) 
						{
							 ...
							 * is remembered now in list[new].
							 * allo_list(new,&newmark,blocks) will not allocate any
							 * new space for list[new], nor for list[new]->mark.
							 * The space pointed to by list[new]->sys will be lost, 
		...
		for (i = ineq; i != new; i++) 
		{
			if (sptr+sysrow > ar3bd) 
			{
				 ...
				 * and recompute list[]->sys, list[]->ptr, sptr, ar3bd and sysp.
				 */
				reallocate(new, &sptr);
				ar3bd = ar3+nel_ar3-1;
				/*
				 * Change by M.S. 3.6.1992:
				 * "sysp" points to the beginning of the space used by list[]->sys.
			 ...
			 */
			row_add(list[i]->sys+1,list[i]->ptr+1,sptr+1,sysrow-1); 
			list[i]->sys = sptr;
			sptr = sptr+sysrow;        
		}
		...
		{
			/*
			 * Move the 0-inequalities to the front of list[].
			 * Move also the whole vector list[]->sys for such an inequality
			 * to the front of array "ar3",
			 * overwriting the positive inequalities.
			 * Since list[j]->sys < list[i]->sys for any two "0"inequalities j < i,
			 * one may move the contents of list[]->sys sequentially.
			 * list[j]->sys[0] is not up to date after this operation,
			...
			{ 
				/* save 0 ieqs */
				sptr++; 
				iep = list[j]; list[j] = list[i]; list[i] = iep;
				for (col = 1; col <= nieq; col++,sptr++)   
					(*RAT_assign)(sptr,list[j]->sys+col);
				list[j]->sys = sptr - sysrow;
			}
			neg = j;
		}
		else 
			/*
			 * Move the vectors list[]->sys to the front of array "ar3"
			...
			{
				sptr++; 
				if (j == neg || (list[i]->sys < list[j]->sys && i != zer)) 
				{
					/* first try to save as many + inequalities as possible */
					for (col = 1; col <= nieq; col++,sptr++)   /* save + ieqs */
						(*RAT_assign)(sptr,list[i]->sys+col);
					list[i]->mark[(points-itr)/32+1] |= nmark;
					/* 
				...
				else   
				{
					...
					list[j]->sys = sptr - sysrow;
					/* 
					 * Rotate the contents of list[i,...,j] to the right: link[j,i,...].
					 * Thereby list[k]->sys < list[l]->sys is preserved for k < l
					 * of the same type.
					 */
					iep = list[j];
					for (n = j; n > i; n--)
		 ...
		 * In list[]->sys, divide the numerators by their gcd,
		...
		for (i = 0,sptr=sysp; i < ineq;i++,sptr++)
			for (j=0,sptr=list[i]->sys+1; j < nieq;j++,sptr++)
				size_info(sptr,&nz,&ld);
		...
		/* 
		 * Change by M.S. 4.6.92:
		 * When ineq=0 or sysrow=0, nz is divided by 0.
		 * Change by A.L. 02.12.93:
		 * a larger output format
		 fprintf(prt,"|%7i |%5i |%4c |%7.2f |%7i |%8.2f |",
		 ineq,ld,MP_realised?'y':'n',(float)nz/(float)(ineq*sysrow),total_size/1000,time_used());
		 */
		...
		/* 17.01.1994: include logging on file porta.log */
		log( "|%9i |%5i |%4c |%7.2f |%8i |%10.2f |%10.2f |",
				ineq,ld,MP_realised?'y':'n',
				(ineq && sysrow ? (float)nz/(float)(ineq*sysrow) : 0),
				total_size/1000,time_used(), total_time());
		if (!MP_realised && is_set(Opt_elim))
	...
	sysrow = rowl-niterat;
	/* 
	 * sysrow was the length of the list[]->sys vectors, which is nieq+1.
	 * rowl-niterat is the number of noneliminated variables+1 in case of "fmel",
	 *              is dim + 1 - equa			     in case of "traf".
	 * Now the inequalities are computed by multiplying list[]->sys
	 * with matrix "ar2".
	...
	if (sysrow > nieq+1) 
	{
		 ...
		 *
		 ...
		 for (j = 0; j < nieq+1; j++) 
		 (*RAT_assign)(sptr+j,list[i]->sys+j);
		 list[i]->sys = sptr;
		 }
	 ...
	 * niterat+1, ..., dim.
	 * 
	 * Compute the product of list[]->sys with the original inequalities,
	 * but only for the non-eliminated variables and the right-hand side.
	 * The result is stored one vector to the left of list[]->sys.
	 * (This is the reason for the free space at the beginning of ar3).
	 */
	for (i = 0; i < ineq; i++,sptr += sysrow)  
	{
		for (j = 0; j < sysrow; j++)
			vecpr(iesp+nieq*((elim_ord)?elim_ord[j]:j), list[i]->sys+1, sptr+j, nieq);
		list[i]->sys = sptr;
		/* 
	...
	/* 17.01.1994: include logging on file porta.log */
	log( "sum of inequalities over all iterations : %6i\n",totalineq);
	log( "maximal number of inequalities          : %6i\n\n",maxnumineq);
	...
	/* This should not done, because the value of ar3 is assigned to
	   list[~]->sys. Reallocating causes a free memory read in
	   origin_add(). 
...
void red_test(indx,inieq,rowl_inar)
	 int indx[];
	 RAT *inieq;
	 int *rowl_inar;
	...
	*rowl_inar = dim-equa+1;
	...
	for (ie1 = 0; ie1 != ineq; ie1++) 
	{
		...
		if (ncv + nce > 0) 
		{
			...
			for (ie2 = 0; ie2 != ineq; ie2++) 
			{
				if (ie1 != ie2) 
				{
					if (eqie_satisfied(list[ie2]->sys,mid,sysrow,0)) 
					{
						fprintf (prt,"r");
	...
	for (i = 0; i < ineq; i++) 
	{
		...
		for (j = 0; j < sysrow; j++, pp++)
			(*RAT_assign)(pp, list[i]->sys+j);
		list[i]->sys = list[i]->ptr;
	}
...
void rmlistel(blocks,cel,lastie,real,sysrow)
	 int blocks,cel,lastie,real,sysrow;

/*****************************************************************/
/*
 * Remove element cel from list.
 * If real = 1,
 * move the contents of list[cl+i]->sys and list[cl+i]->mark physically 
 * to list[cl+i-1]->sys, and list[cl+i-1]->mark, for i = 1, ..., lastie.
 * Otherwise,
 * rotate the contents of list[cl, ..., lastie] to the left.
 */
{
	listp lp;
	int i,j;
	...
	if (real) 
	{
		for (i = cel; i < lastie; i++) 
		{
			lp = list[i];
			for (j = 0; j <= sysrow; j++)
				(*RAT_assign)(lp->sys+j, list[i+1]->sys+j);
			for (j = 0; j < blocks; j++)
				lp->mark[j] = list[i+1]->mark[j];
		}
	}
	else 
	{ 
...
/*****************************************************************/
{
	for (;n;n--,p++) 
	{
		printf("%i/%i ",p[0].num,p[0].den.i);
	
		/* 17.01.1994: include logging on file porta.log */
		log( "%i/%i ",p[0].num,p[0].den.i );
	}
	printf("\n");
